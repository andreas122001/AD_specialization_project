...
class GlobalConfig:
    ...
    def __init__(self):
        ...
        if self.crop_bev and not self.crop_bev_height_only_from_behind:
            ...
            )  # If we cut the bev semantics symetrically, we also need a symmetric lidar range
        self.min_y = -32
        self.max_y = 32
        self.min_z = -4
        self.max_z = 4
        ...
        self.target_speed_bins = [
            x + 0.001 for x in self.target_speeds[1:]
        ]  # not used with two hot encodings
        ...
        self.learn_origin = (
            1  # Whether to learn the origin of the waypoints or use 0 / 0
        )
        ...
        self.detailed_loss_weights = {
            ...
            "loss_trajectories": 1.0,
            "loss_trajectory_confidence": 1.0,
            "loss_depth": 1.0,
        ...
        self.grad_clip_max_norm = (
            1.0  # Max value for the gradients if gradient clipping is used.
        )
        ...
        self.use_depth = (
            True  # Whether to use depth prediction as auxiliary loss for training.
        )
        self.num_repetitions = 1  # How many repetitions of the dataset we train with.
        self.continue_epoch = (
            True  # Whether to continue the training from the loaded epoch or from 0.
        )

        self.smooth_route = True  # Whether to smooth the route points with a spline.
        self.ignore_index = (
            -999
        )  # Index to ignore for future bounding box prediction task.
        ...
        self.use_label_smoothing = (
            False  # Whether to use label smoothing in the classification losses
        )
        self.use_twohot_target_speeds = (
            True  # Whether to use two hot encoding for the target speed classification
        )
        ...
        self.clip_delta = (
            1.0  # maximum change in speed input to longitudinal controller
        )
        ...
        self.use_semantic = (
            True  # Whether to use semantic segmentation as auxiliary loss
        )
        ...
        self.deconv_channel_num_0 = (
            128  # Number of channels at the first deconvolution layer
        )
        self.deconv_channel_num_1 = (
            64  # Number of channels at the second deconvolution layer
        )
        self.deconv_channel_num_2 = (
            32  # Number of channels at the third deconvolution layer
        )
        ...
        self.use_discrete_command = (
            True  # Whether to input the discrete target point as input to the network.
        )
        self.add_features = True  # Whether to add (true) or concatenate (false) the features at the end of the backbone.

        self.image_u_net_output_features = (
            512  # Channel dimension of the up-sampled encoded image in bev_encoder
        )
        self.bev_latent_dim = (
            32  # Channel dimensions of the image projected to BEV in the bev_encoder
        )
        ...
        self.num_transformer_decoder_layers = (
            6  # Number of layers in the TransFormer decoder
        )
        ...
        self.wp_dilation = (
            1  # Factor by which the wp are dilated compared to full CARLA 20 FPS
        )

        self.extra_sensor_channels = (
            128  # Number of channels the extra sensors are embedded to
        )
        ...
        self.multi_wp_output = (
            False  # Predicts 2 WP outputs and uses the min loss of both.
        )
        ...
        self.plant_object_types = (
            6  # vehicle, pedestrian, traffic light, stop sign, route, other
        )
        self.plant_num_attributes = (
            7  # x,y, extent x, extent y,yaw,speed, brake, (class)
        )
        ...
        self.plant_max_speed_pred = (
            60.0  # Maximum speed we classify when forcasting cars.
        )
    ...
    def initialize(self, root_dir="", setting="all", **kwargs):
        ...
        for td_path in self.root_dir:
            self.data_roots = self.data_roots + [
                os.path.join(td_path, name) for name in os.listdir(td_path)
            ]
