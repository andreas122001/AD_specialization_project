...
class CARLA_Data(Dataset):  # pylint: disable=locally-disabled, invalid-name
    ...
    def __getitem__(self, index):
        ...
        loaded_lidars = []
        loaded_boxes = []
        loaded_future_boxes = []  # can potentially use for trajectory prediction
        loaded_measurements = []
        ...
        for i in range(self.config.seq_len):
            measurement_file = str(measurements[0], encoding="utf-8") + (
        ...
        for i in range(self.config.seq_len):  # Load data from disc
            ...
            # Load current and future bounding boxes
            if self.config.detect_boxes:
                boxes_i = self._load_json_gz(str(boxes[i], encoding="utf-8"))
            ...
            images_i = self._load_jpg(str(images[i], encoding="utf-8"))

            if self.config.use_semantic:
                semantics_i = self._load_png(
                    str(semantics[i], encoding="utf-8")
                )
            if self.config.use_bev_semantic:
                bev_semantics_i = self._load_png(
                    str(bev_semantics[i], encoding="utf-8"), crop=False
                )

            if self.config.use_depth:
                depth_i = self._load_png(str(depth[i], encoding="utf-8"))

            if self.config.augment:
                images_augmented_i = self._load_jpg(
                    str(images_augmented[i], encoding="utf-8")
                )

                if self.config.use_semantic:
                    semantics_augmented_i = self._load_png(
                        str(semantics_augmented[i], encoding="utf-8")
                    )

                if self.config.use_bev_semantic:
                    bev_semantics_augmented_i = self._load_png(
                        str(bev_semantics_augmented[i], encoding="utf-8"), crop=False
                    )

                if self.config.use_depth:
                    depth_augmented_i = self._load_png(
            ...
            if self.config.use_bev_semantic:
                ...
                if self.config.pixels_per_meter == 4.0:
                    bev_semantics_i = (
                        bev_semantics_i[64:192, 64:192]
                        .repeat(2, axis=0)
                        .repeat(2, axis=1)
                    )
                    bev_semantics_augmented_i = (
                        bev_semantics_augmented_i[64:192, 64:192]
                        .repeat(2, axis=0)
                        .repeat(2, axis=1)
                    )
        ...
        if (
            ...
            and loaded_images_augmented[self.config.seq_len - 1] is not None
        ):
            aug_rotation = current_measurement["augmentation_rotation"]
            aug_translation = current_measurement["augmentation_translation"]
            loaded_images_choice = loaded_images_augmented
            loaded_semantics_choice = loaded_semantics_augmented
            loaded_bev_semantics_choice = loaded_bev_semantics_augmented
            loaded_depth_choice = loaded_depth_augmented
        else:
            aug_rotation = 0.0
            aug_translation = 0.0
            loaded_images_choice = loaded_images
            loaded_semantics_choice = loaded_semantics
            loaded_bev_semantics_choice = loaded_bev_semantics
            loaded_depth_choice = loaded_depth

        # TODO loading by static index, we want to load all images, not just this one
        # TODO need to loop over seq_len
        # for idx in range(self.config.seq_len):
        #   image_i = load_image(path)
        #   image_i = preprocess(image_i)  # apply augments
        #   images.append(image_i)
        # data["rgb"] = np.concatenate(images, axis=0)

        # Here we load images, semantic, bev_Semantic and depth
        if self.config.use_color_aug:
            processed_image = self.image_augmenter_func(
                image=loaded_images_choice[self.config.seq_len - 1]
            )
        else:
            processed_image = loaded_images_choice[self.config.seq_len - 1]

        if self.config.use_semantic:
            semantics_i = self.converter[
                loaded_semantics_choice[self.config.seq_len - 1]
            ]  # pylint: disable=locally-disabled, unsubscriptable-object
        if self.config.use_bev_semantic:
            bev_semantics_i = self.bev_converter[
                loaded_bev_semantics_choice[self.config.seq_len - 1]
            ]  # pylint: disable=locally-disabled, unsubscriptable-object
        if self.config.use_depth:
            # We saved the data in 8 bit and now convert back to float.
            depth_i = (
                loaded_depth_choice[self.config.seq_len - 1].astype(np.float32)
                / 255.0
            )  # pylint: disable=locally-disabled, unsubscriptable-object
        ...
        target_speed_twohot = self.get_two_hot_encoding(
            current_measurement["target_speed"], self.config.target_speeds, brake
        )
        ...
        lidar_bev = self.lidar_augmenter_func(
            image=np.transpose(lidar_bev, (1, 2, 0))
        )
        ...
        data["next_command"] = t_u.command_to_one_hot(
            current_measurement["next_command"]
        )
        ...
        target_point = self.augment_target_point(
            target_point, y_augmentation=aug_translation, yaw_augmentation=aug_rotation
        )
    ...
    def _load_png(self, path: Union[str, os.PathLike], crop: bool = True) -> np.ndarray:
        "Used for load semantics, bev_semantics and depth"
...
class CARLA_Data_tar_routes(CARLA_Data):
    ...
    def __init__(
        ...
        for sub_root in tqdm(root, file=sys.stdout, disable=rank != 0):
            ...
            # list subdirectories in root
            # routes = next(os.walk(sub_root))[1]
            routes = [r.split(".")[0] for r in next(os.walk(sub_root))[2]]

            for route in tqdm(
                routes, position=1, leave=False
            ):  # loop over individual routes within this scenario folder
                total_routes += 1
                route_dir = sub_root + "/" + route

                with tarfile.open(route_dir + ".tar", "r:") as route_tar:
                    route_files = [f.path for f in route_tar.getmembers() if f.isfile()]
                    # We skip data where the expert did not achieve perfect driving score (except for min speed infractions)
                    if not self._is_valid_route(route_dir, route_tar):
                        skipped_routes += 1
                        continue
                    trainable_routes += 1
                ...
                #     os.listdir(lidar_dir)
                # )  # How many frames recorded for the current route
                num_seq = len([f for f in route_files if "lidar" in f])
    ...
    def _is_valid_route(self, route_dir: Union[os.PathLike, str], route_tar) -> bool:
        """
        Returns True if the route meets the success conditions (perfect score, not failed, etc.)
        """
        if route_dir.split("/")[-1].startswith("FAILED_"):
            return False

        # Check results file
        # with tarfile.open(route_dir + ".tar", "r:*") as route_tar:
        # Extract file from archive and uncompress with gzip
        try:
            # Check if lidar dir exists in tar members
            all_member_paths = [m.path for m in route_tar.getmembers()]
            lidar_dir = route_dir + "/lidar"
            # tarfile removes prefix slash for members
            if lidar_dir.removeprefix("/") not in all_member_paths:
                return False

            results_path = f"{route_dir}/results.json.gz".removeprefix("/")

            results_member = route_tar.getmember(results_path)
            results_file = route_tar.extractfile(results_member)
    ...
    def _load_lidar(self, path: Union[str, os.PathLike]) -> np.ndarray:
        las_bytes = self._load_tar_member_as_bytes(path)
        las_object = laspy.read(las_bytes)
        lidars_i = las_object.xyz
        return lidars_i

    def _load_jpg(self, path: Union[str, os.PathLike]) -> np.ndarray:
        member_bytes = self._load_tar_member_as_bytes(path)
        data_array = np.frombuffer(member_bytes, dtype=np.uint8)
        images_i = cv2.imdecode(data_array, cv2.IMREAD_COLOR)  # Returns BGR format
    ...
    def _load_png(
        self, path: Union[str, os.PathLike], crop: bool = True
    ) -> np.ndarray:
        member_bytes = self._load_tar_member_as_bytes(path)
        data_array = np.frombuffer(member_bytes, dtype=np.uint8)
        semantics_i = cv2.imdecode(data_array, cv2.IMREAD_UNCHANGED)

        if crop:
            semantics_i = t_u.crop_array(self.config, semantics_i)
        return semantics_i

    def _load_png(self, path: Union[str, os.PathLike]) -> np.ndarray:
        member_bytes = self._load_tar_member_as_bytes(path)
        data_array = np.frombuffer(member_bytes, dtype=np.uint8)
        bev_semantics_i = cv2.imdecode(data_array, cv2.IMREAD_UNCHANGED)
    ...
    def _load_png(self, path: Union[str, os.PathLike]) -> np.ndarray:
        member_bytes = self._load_tar_member_as_bytes(path)
        data_array = np.frombuffer(member_bytes, dtype=np.uint8)
        depth_i = cv2.imdecode(data_array, cv2.IMREAD_UNCHANGED)  # Returns BGR format

        depth_i = t_u.crop_array(self.config, depth_i)
        return depth_i

    def _load_json_gz(self, path: Union[str, os.PathLike]) -> np.ndarray:
        boxes_i = self._load_json_gzip_from_tar(path)
        return boxes_i

    def _load_json_gz(self, path: Union[str, os.PathLike]) -> np.ndarray:
        measurements_i = self._load_json_gzip_from_tar(path)
        return measurements_i

    # Loading of tarfiles
    def _load_tar_member_as_bytes(self, path: Union[str, os.PathLike]):
        archive_path = "/".join(path.split("/")[:-2])
        with tarfile.open(archive_path + ".tar", "r") as tar:
            file_obj = tar.extractfile(path.removeprefix("/"))
            file_bytes = file_obj.read()

        return file_bytes

    def _load_json_gzip_from_tar(self, path: Union[str, os.PathLike]):
        archive_path = "/".join(path.split("/")[:-2])
        with tarfile.open(archive_path + ".tar", "r") as tar:
            file_obj = tar.extractfile(path.removeprefix("/"))
            with gzip.open(file_obj, "rt", encoding="utf-8") as f:
                member_json = ujson.load(f)
...
if __name__ == "__main__":
    ...
    config.initialize(
        root_dir=[
            "/cluster/work/andrebw/repos/temporal_garage/results/data/garage_v2_2025_03_21/data"
        ]
    )
    dataset = CARLA_Data(
