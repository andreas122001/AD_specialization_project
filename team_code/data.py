            ...
            if not self.data_cache is None and cache_key in self.data_cache:
                ...
                if not self.config.use_plant:
                    ...
                    if self.config.use_bev_semantic:
                        bev_semantics_i = cv2.imdecode(
                            bev_semantics_i, cv2.IMREAD_UNCHANGED
                        )
                    if self.config.use_depth:
                        depth_i = cv2.imdecode(depth_i, cv2.IMREAD_UNCHANGED)
                    if self.config.augment:
                        images_augmented_i = cv2.imdecode(
                            images_augmented_i, cv2.IMREAD_UNCHANGED
                        )
                        if self.config.use_semantic:
                            semantics_augmented_i = cv2.imdecode(
                                semantics_augmented_i, cv2.IMREAD_UNCHANGED
                            )
                        if self.config.use_bev_semantic:
                        ...
                        if self.config.use_depth:
                            depth_augmented_i = cv2.imdecode(
                                depth_augmented_i, cv2.IMREAD_UNCHANGED
                            )
            ...
            # Load data from the disc
                ...
                if not self.data_cache is None:
                    ...
                    if not self.config.use_plant:
                        ...
                        if self.config.use_bev_semantic:
                            _, compressed_bev_semantic_i = cv2.imencode(
                                ".png", bev_semantics_i
                            )
                        if self.config.use_depth:
                            _, compressed_depth_i = cv2.imencode(".png", depth_i)
                        if self.config.augment:
                            _, compressed_image_augmented_i = cv2.imencode(
                                ".jpg", images_augmented_i
                            )
                            if self.config.use_semantic:
                                _, compressed_semantic_augmented_i = cv2.imencode(
                                    ".png", semantics_augmented_i
                                )
                            if self.config.use_bev_semantic:
                                _, compressed_bev_semantic_augmented_i = cv2.imencode(
                                    ".png", bev_semantics_augmented_i
                                )
                            if self.config.use_depth:
                                _, compressed_depth_augmented_i = cv2.imencode(
                                    ".png", depth_augmented_i
                                )
                        ...
                        with laspy.open(
                            ...
                            point_record = laspy.ScaleAwarePointRecord.zeros(
                                lidars_i.shape[0], header=header
                            )
            ...
            if self.config.augment and augment_sample:
                if self.config.use_color_aug:
                    processed_image = self.image_augmenter_func(
                        image=loaded_images_augmented[self.config.seq_len - 1]
                    )
                else:
                ...
                if self.config.use_depth:
                    ...
                    depth_i = (
                        loaded_depth_augmented[self.config.seq_len - 1].astype(
                            np.float32
                        )
                        / 255.0
                    )  # pylint: disable=locally-disabled, unsubscriptable-object
                ...
                if self.config.use_color_aug:
                    processed_image = self.image_augmenter_func(
                        image=loaded_images[self.config.seq_len - 1]
                    )
                else:
