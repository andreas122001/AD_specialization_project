...
class DataAgent(AutoPilot):
    ...
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
        ...
        if self.save_path is not None and self.datagen:
            ...
            (self.save_path / "rgb_backward").mkdir()
    ...
    def _init(self, hd_map):
        ...
        self._local_planner = LocalPlanner(
            self._vehicle, opt_dict={}, map_inst=self.world_map
        )
        ...
        if self.save_path is not None and (self.datagen or self.tmp_visu):
            result += [
                ...
                },
                {
                    ...
                    "roll": self.config.camera_rot_0[0],
                    "pitch": self.config.camera_rot_0[1],
                    "yaw": self.config.camera_rot_0[2] + 180,
                    "width": self.config.camera_width,
                    "height": self.config.camera_height,
                    "fov": self.config.camera_fov,
                    "id": "rgb_backward",
    ...
    def tick(self, input_data):
        ...
        if self.save_path is not None and (self.datagen or self.tmp_visu):
            ...
            rgb_backward = input_data["rgb_backward"][1][:, :, :3]
    ...
    def save_sensors(self, tick_data):
        ...
        # Just in case we are using it later
        # cv2.imwrite(
        #     str(self.save_path / "rgb_backward" / (f"{frame:04}.jpg")),
        #     tick_data["rgb_backward"],
        # )
    ...
    def destroy(self, results=None):
        ...
        if results is not None and self.save_path is not None:
            with gzip.open(
                os.path.join(self.save_path, "results.json.gz"), "wt", encoding="utf-8"
            ) as f:
    ...
    def get_bounding_boxes(self, lidar=None):
        ...
        for static in static_list:
            if (
                static.get_location().distance(self._vehicle.get_location())
                < self.config.bb_save_radius
            ):
