...
class CARLA_Data(Dataset):  # pylint: disable=locally-disabled, invalid-name
        ...
        for sub_root in tqdm(root, file=sys.stdout, disable=rank != 0):
            ...
            for (
                ...
                for seq in range(first_frame, last_frame):
                    ...
                    if prune:
                        for idx in range(
                            ...
                            measurements_i = self._load_json_gz(
                                measurement[-1] + f"/{(seq+idx):04}.json.gz"
                            )
                    ...
                    # If heuristic is false, we ignroe the sequence
                    if prune and not self._pruning_heuristic(measurements):
                        continue
    ...
    def _pruning_heuristic(self, measurements: list[dict]):
        ...
        # return abs(max(angles) - min(angles)) > angle_threshold
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.config.seq_len):
            measurement_file = str(measurement_root, encoding="utf-8") + (
                # TODO: confirm i*self.config.seq_step
    ...
    def _load_cached(self, path: Union[str, os.PathLike], load_func):
        ...
        if self.data_cache is not None and path in self.data_cache:
            image_i = self.data_cache[path]
        else:
            image_i = load_func(path)
            # save to cache, if enabled
            if self.data_cache is not None:
                self.data_cache[path] = image_i
        return image_i
